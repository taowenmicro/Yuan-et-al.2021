...
res1path <- paste(result_path,"/result3",sep = "")
dir.create(res1path)
...
##--leaf microbiol community analyse #-----------
subpath <- paste(res1path,"/result_leaf_micro",sep = "")
dir.create(subpath)
...
alp = alpha(ps = ps_leaf,inde="Shannon",group = "Group",Plot = TRUE )
index= alp[[3]]
head(index)
...
result = BetaDiv(ps = ps_leaf, group = "Group", dist = "bray", method = "PCoA", Micromet = "adonis", pvalue.cutoff = 0.05)
...
dir.create(barpath)
j = "Phylum"
result = barMainplot(ps = ps_leaf,j = "Phylum",rep = 6,axis_ord = NULL,label = FALSE ,sd = FALSE,Top = 10)
...
result = ggClusterNet::network(ps = ps_leaf,N = 0.001,r.threshold=0.6,p.threshold=0.05,label = FALSE,path = netpath ,zipi = TRUE)
# ?network
# 全部样本的网络比对
...
# library(patchwork)
# p_result1_1 <- p1_1 |(p2_1)|p3_1|p4_1
# p_result1_1 
...
EEEEEEEEEEEEE
"
p_result3_1 <- p1_1 + p2_3 + p3_2 + p0_2 + p4_1 +
  plot_layout(design = layout)

# sp1 <- p1_1 |(p2_3/p0_2)|p3_1
# sp1/p4_1

plotname = paste(res1path,"/result3.pdf",sep = "")
ggsave(plotname, p_result3_1 ,width = 18,height = 12)
plotname = paste(res1path,"/result3_1.pdf",sep = "")
ggsave(plotname, p_result3_1 ,width = 18,height = 15)
...
#--------leaf GC analyses plot #------------

subpath <- paste(res1path,"/result_leaf_GC",sep = "")
dir.create(subpath)
...
barpath = paste(subpath,"/GC_group_barplot/",sep = "")
dir.create(barpath)
...
#--提取图片
g1_1 = res[[1]] + scale_fill_manual(values = colset3) +  scale_x_discrete(limits = axis_order) + mytheme1
g1_1
#--提取数据
plotdata = res[[2]]
...
betapath = paste(subpath,"/GCbeta/",sep = "")
dir.create(betapath)
...
#---------数据基本整理-这里是为了计算多样性
wlxpath = paste(subpath,"/wlxtest/",sep = "")
dir.create(wlxpath)
#-峰面积去整数-保留一位小数
psG  = transform_sample_counts(psG, function(x) x / sum(x) );psG
#-----人工指定分组信息
group1 = c("Control","OF")
group2 = c("Control","CF")
...
result = statSuper(ps = psG,group  = "Group",artGroup = b,method = "wilcox")
head(result)
...
result$EI.spectrum = NULL

colnames(result)
difres <- result %>%  dplyr::filter(Control_CF_fdr < 0.05 & Control_OF_fdr < 0.05 & Metabolite.name != "Unknown"& Metabolite.name != "unknown"   )
dim(difres)
...
data
plotname1 = paste(netpath,"/network_all.jpg",sep = "")
ggsave(plotname1, g4_1,width = 10,height = 5)
plotname1 = paste(netpath,"/network_all1.pdf",sep = "")
ggsave(plotname1, g4_1,width = 10,height = 9)

tablename <- paste(netpath,"/co-occurrence_Grobel_net",".csv",sep = "")
write.csv(data,tablename)

library(patchwork)
# p_result1_2 <-(g2_3 + g1_1 )/( g4_1)
...
g3_1 = as.ggplot(g3_1)
p_result3_2 <- g2_3 + g1_1 + g3_1 + g4_1 +
  plot_layout(design = layout)
...
plotname = paste(res1path,"/result3_2.pdf",sep = "")
ggsave(plotname, p_result3_2,width = 15,height = 10)

plotname = paste(subpath,"/result1_2.pdf",sep = "")
ggsave(plotname, p_result1_2,width = 20,height = 10)
...
head(dataenv)
#
result = MuiKwWlx(data = dataenv,num = c(5:9))
result
...
result1 = FacetMuiPlotresultBox(data = dataenv,num = c(5:9),result = result,sig_show ="abc",ncol = 5 )
p1_1 = result1[[1]] + scale_x_discrete(limits = axis_order) + 
  theme_bw() + mytheme1 +
  guides(fill = guide_legend(title = NULL)) +
  scale_fill_manual(values = colset1)
...
res = FacetMuiPlotresultBar(data = dataenv,num = c(5:9),result = result,sig_show ="abc",ncol = 5)
p1_2 = res[[1]]+ scale_x_discrete(limits = axis_order) + guides(color = FALSE) + theme_bw()  + 
  mytheme1+ guides(fill = guide_legend(title = NULL))+
  scale_fill_manual(values = colset1)
p1_2

res = FacetMuiPlotReBoxBar(data = dataenv,num = c(5:9),result = result,sig_show ="abc",ncol = 5)
p1_3 = res[[1]]+ scale_x_discrete(limits = axis_order) + theme_bw()  + mytheme1 + guides(fill = guide_legend(title = NULL))+
  scale_fill_manual(values = colset1)
p1_3
...
ggsave(FileName, p1_1, width = 18, height = 4)
...
result = RDA_CCA(ps = ps_leaf,env = env[c(-1,-2)],path = envpath)
...
ps_rela  = transform_sample_counts(ps_leaf, function(x) x / sum(x) );ps_rela 
...
out
plotname = paste( envpath,"/vpa_out.pdf",sep = "")
...
exp
plotname = paste( envpath,"/vpa_explain.pdf",sep = "")
...
netpath = paste(subpath,"/three_netwotk_bios/",sep = "")
dir.create(netpath)
row.names(env) <- env$ID
env$ID = NULL
...
data = result[[2]]
plotname1 = paste(netpath,"/network_all.jpg",sep = "")
ggsave(plotname1, p,width = 12,height = 6)
plotname1 = paste(netpath,"/network_all1.pdf",sep = "")
ggsave(plotname1, p,width = 12,height = 4 )
tablename <- paste(netpath,"/co-occurrence_Grobel_net",".csv",sep = "")
write.csv(data,tablename)
